...
| `openai_images`    | create    | `POST /v1/images/generations` | JSON                          |
| `openai_images`    | edit      | `POST /v1/images/edits`   | JSON or multipart                 |
...
`openai_images` defines a single `"sync"` mode: the create and edit calls block until the task is terminal, so the claim is always `{name: "openai_images", supports: ["sync"]}` and the plugin exports `protocols.openai_images.decodeRequest` and `renderFinal` plus the `listArtifacts` / `buildContentRequest` driver hooks. One `decodeRequest` serves both operations and distinguishes them by `ctx.operation` (`create` or `edit`); edit bodies are the OpenAI multipart form (`image`, optional `mask`, text fields) or a JSON object, and uploaded files reach the driver only as `FileReference` values for `buildSubmitRequest` placeholders. There is no retrieval binding; the generic task API (`GET /v1/tasks/:task_id`, `/artifacts`) covers slow or timed-out generations. `renderFinal` returns `{data: [{url?, b64_json?, artifact?, revised_prompt?}]}`. `url` normally carries host-injected `ctx.artifacts[key].url`; `artifact` names one of the plugin's own image artifact keys and asks the host to fetch that artifact through `buildContentRequest` (same SSRF and channel policy as the content endpoint, 32 MiB per image, 64 MiB per response) and emit its bytes as `b64_json`, which is how a plugin honours the client's `response_format`. The host adds `created`, requires every item to carry exactly one of `url`, `b64_json` or `artifact`, requires `url` to be an absolute HTTP(S) URL, and rejects any other top-level or item field. A failed task returns HTTP 500 with error code `task_failed`; an observation timeout returns HTTP 504 with `task_timeout`; a completed task whose images could not be rendered or inlined (for example the artifact fetch was rejected by the gateway fetch policy) returns HTTP 502 with `image_fetch_failed` while the task and its settlement stand. All messages carry the gateway task id and never the upstream failure detail, which goes to the server log. `stream: true` and `background: true` are rejected with 400 before any hook runs. Models that no plugin claims on this path fall through to the built-in image relay.
