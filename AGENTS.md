...
- For row-level locks in GORM v2, use `model.lockForUpdate(tx)` instead of `tx.Set("gorm:query_option", "FOR UPDATE")`. The legacy `gorm:query_option` form is ignored by GORM v2 and does not lock rows; `lockForUpdate` emits `FOR UPDATE` for MySQL/PostgreSQL and safely skips it on SQLite.
